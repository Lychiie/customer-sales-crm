(() => {
  ...
  style.textContent = `
    ...
    .dn-brand{display:flex;align-items:center;gap:3mm;margin-bottom:3mm}.dn-company-logo{width:18mm;height:18mm;object-fit:contain;flex:none;print-color-adjust:exact}.dn-brand>div{min-width:0}.dn-company h2{font-size:19px;line-height:1.55;margin:3mm 0}.dn-address{white-space:pre-wrap;overflow-wrap:anywhere}.dn-kicker{font:600 10px/1.6 Arial,sans-serif;letter-spacing:2px;color:#536174}
  ...
  const buildSheet = (issuer, doc, items) => `<article class="dn-sheet" aria-label="ใบส่งของ"><header class="dn-head"><div class="dn-company"><div class="dn-brand"><img class="dn-company-logo" src="company-logo.png" alt="โลโก้บริษัท"><div><div class="dn-kicker">DELIVERY DOCUMENT</div><h2>${escape(issuer.name || '-')}</h2></div></div><p class="dn-address">${escape(window.DocumentAddress.format(issuer.address) || '-')}</p><small>เลขประจำตัวผู้เสียภาษี ${escape(issuer.tax_id || '-')}</small></div><div class="dn-title"><h2>ใบส่งของ</h2><p class="dn-kicker">DELIVERY NOTE</p><div class="dn-meta"><p><span>เลขที่เอกสาร</span><strong>${escape(doc.document_number || 'ตัวอย่าง')}</strong></p><p><span>วันที่ส่งสินค้า</span><strong>${date(doc.issue_date)}</strong></p></div></div></header><section class="dn-parties"><div class="dn-party"><h3>ลูกค้า / CUSTOMER</h3><strong>${escape(doc.customer_name)}</strong><p>${escape(window.DocumentAddress.format(doc.customer_address) || '-')}</p><small>เลขประจำตัวผู้เสียภาษี ${escape(doc.customer_tax_id || '-')}</small></div><div class="dn-party dn-shipping"><h3>สถานที่จัดส่ง / SHIP TO</h3><p>${escape(doc.shipping_address)}</p></div></section><div class="dn-section-label"><b>รายการสินค้าที่จัดส่ง</b><span>${items.length} รายการ</span></div><table><colgroup><col style="width:8%"><col style="width:66%"><col style="width:14%"><col style="width:12%"></colgroup><thead><tr><th>ลำดับ<small>NO.</small></th><th>รายการสินค้า / ขนาด<small>DESCRIPTION / SIZE</small></th><th>จำนวน<small>QUANTITY</small></th><th>หน่วย<small>UNIT</small></th></tr></thead><tbody>${items.map((item,i)=>`<tr><td>${i+1}</td><td class="dn-description">${escape([item.name,item.specification].filter(value => value != null && String(value).trim()).join(' '))}</td><td>${Number(item.quantity).toLocaleString('th-TH',{maximumFractionDigits:3})}</td><td>${escape(item.unit)}</td></tr>`).join('')}${Array.from({length:Math.max(0,12-items.length)},()=>'<tr class="dn-empty" aria-hidden="true"><td></td><td></td><td></td><td></td></tr>').join('')}</tbody></table><footer><div class="dn-note"><b>หมายเหตุ / REMARKS</b>${escape(doc.notes || '-')}</div><div class="dn-sign">${[['ผู้รับสินค้า','RECEIVED BY'],['ผู้ส่งสินค้า','DELIVERED BY'],['ผู้อนุมัติ','AUTHORIZED BY']].map(([th,en])=>`<div><div class="dn-sign-line"></div><p>${th}</p><small>${en}</small><p class="dn-sign-date">วันที่ ........ / ........ / ........</p></div>`).join('')}</div><div class="dn-end"><span>ใบส่งของ / DELIVERY NOTE</span><span>${escape(doc.document_number || 'ตัวอย่าง — ยังไม่ได้บันทึก')}</span></div></footer></article>`;

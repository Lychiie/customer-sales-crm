...
(() => {
  ...
  const mount = (root, customers, products) => {
    ...
    const update=()=>{
      ...
      try {const t=calculate(read(),products);root.querySelector('.qe-total').textContent=`รวม ${money(t.subtotal)} บาท • ส่วนลด ${money(t.discount_amount)} บาท\nก่อน VAT ${money(t.taxable_amount)} บาท • VAT 7% ${money(t.vat_amount)} บาท • ยอดสุทธิ ${money(t.grand_total)} บาท`;}catch(e){root.querySelector('.qe-total').textContent=e.message;}
    };
    const add=()=>{

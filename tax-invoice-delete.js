(() => {
  ...
  const remove=async(request,org,doc,confirmation)=>{
    ...
    pending.add(doc.id);
    try{
      const result=await request('/rest/v1/rpc/delete_tax_invoice',{method:'POST',body:JSON.stringify({p_org:org,p_id:doc.id,p_number:doc.document_number})});
  ...
  const ask=(request,org,doc,onDeleted)=>{
    ...
    input.oninput=()=>{confirm.disabled=input.value!=='ลบ';};
    confirm.onclick=async()=>{
      if(busy||input.value!=='ลบ')return;

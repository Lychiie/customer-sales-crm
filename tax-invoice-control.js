(() => {
  ...
  const load=async()=>{
    ...
    try{
      ...
      const layout=document.createElement('style');layout.textContent='main:has(#tax-invoice-control.active-page){min-width:0}#tax-invoice-control{min-width:0;max-width:100%}#tax-invoice-control .settings-card{max-width:none}#tax-invoice-control .page-toolbar{flex-wrap:wrap;gap:16px}#tax-invoice-control td small{display:block}#tax-invoice-control .tic-cards{grid-template-columns:repeat(auto-fit,minmax(155px,1fr))}';root.prepend(layout);
      const render=()=>{
  ...
  const open=async(request,org,initialTab)=>{const active=page().classList.contains('active-page');await configure(request,org);if(initialTab)filters.tab=initialTab;window.go('tax-invoice-control');history.replaceState(null,'','#tax-invoice-control');if(!active||initialTab)await load();};
